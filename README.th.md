...
| Node.js 18 / 20 / 22 / 24 | ✅ ทดสอบเมทริกซ์ในเครื่อง | รันเทสต์ในเครื่องบน macOS (ครบทั้งสี่เวอร์ชันหลัก) และบน Linux VPS (Node 22, 2026-09-09) ไม่ใช้ GitHub Actions |
...
ใช้ `init --api-base URL` หากต้องการกำหนด HTTPS API endpoint เอง (อนุญาตให้ใช้ HTTP ได้เฉพาะกับ localhost/127.0.0.1 เท่านั้น) การ uninstall รวมถึง dry-run ของการ install/uninstall ไม่จำเป็นต้องใช้โทเคน และรายการติดตั้งที่เก่าเกินไปจะถูกแจ้งไว้ในส่วนแสดงตัวอย่าง (preview) ของ dry-run ส่วนกระบวนการตรวจสอบสถานะผู้ใช้บน Linux จะค้างอยู่ก็ต่อเมื่อใช้ `--check` หรือเมื่อสถานะของบริการที่ลงทะเบียนไว้อยู่ในสภาพ degraded เท่านั้น
