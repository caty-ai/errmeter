...
ใช้ `init --api-base URL` หากต้องการกำหนด HTTPS API endpoint เอง (อนุญาตให้ใช้ HTTP ได้เฉพาะกับ localhost/127.0.0.1 เท่านั้น) การ uninstall รวมถึง dry-run ของการ install/uninstall ไม่จำเป็นต้องใช้โทเคน และรายการติดตั้งที่เก่าเกินไปจะถูกแจ้งไว้ในตัวอย่างของ dry-run ส่วนกระบวนการตรวจสอบสถานะผู้ใช้บน Linux จะค้างอยู่ก็ต่อเมื่อใช้ `--check` หรือเมื่อสถานะของบริการที่ลงทะเบียนไว้อยู่ในสภาพ degraded (ผิดปกติ) เท่านั้น
