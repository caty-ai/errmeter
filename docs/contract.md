...
Status: **freeze candidate v1.3** for issue #2 (v1.0 → v1.3: three review rounds folded in; see the changelog at the end). Once the owner approves, every section marked **[frozen]** may change only through a new contract issue that bumps the relevant version field. Sections marked **[default]** are tunable defaults that implementations MUST honour but families MAY override in config.
...
| `spool/pending/counters.log` | overflow log (§3.3): one line per occurrence, `<fingerprint>\t<fpv>\t<agent>\t<host>\t<ts>\t<message ≤ 120 chars>\n`, ≤ 256 bytes, appended with `O_APPEND` |
...
- At or above `pending_hard_limit` (20000 files): **counter mode** — emit does not create a new event file; it **appends one line** (≤ 256 bytes, well under the 512-byte POSIX `PIPE_BUF` atomic-append floor; Windows `FILE_APPEND_DATA` writes of this size are likewise not interleaved) to `pending/counters.log`. No lock, no read-modify-write, therefore **no increment can be lost to a race** and the file count no longer grows: counter mode adds bytes to one file (≤ 256 B per occurrence; a million-occurrence storm is 256 MB). Heartbeats in counter mode are upserted into `pending/heartbeat-<agent>@<host>.json` (one file per agent, tmp+rename).
- flush **cuts** the log by renaming it to `counters.<nonce>.log` (atomic; concurrent emits simply start a new `counters.log`), reads it, folds lines per fingerprint into one occurrence comment carrying `counter_ref=<nonce>` and `count=`, and deletes the cut file after every group is on the board. Crash recovery: a cut file whose `<nonce>` already appears as `counter_ref=` on the Issue for that fingerprint is not re-posted for that fingerprint (counter idempotency key); once all its fingerprints are accounted for it is deleted. A cut file older than `lock_stale_sec` with no holder is processed by the next flush like any other. **No pending file is ever moved to `dead/` for capacity reasons.** Disk bound = `pending_hard_limit × 32 KiB` ≈ 640 MB of event files plus the overflow log; in practice far lower because compact mode starts at the soft limit.
...
Idempotency keys the sink MUST honour: event `id` (never two board writes for one id), counter `nonce` (§3.3, `counter_ref=`), `fingerprint` (never two *open* failure records for one fingerprint — with the reconciliation in §5.2), alert `key` (one open alert record per key).
