...
| `init` | `--repo`, `--api-base <url>`, `--family`, `--host`, `--role`, `--force`, `--check` | `0` ok (warnings printed for over-scope); `2` usage error only; `3` token/permission problem (names the failing probe); `4` refused to overwrite |
...
Successful install atomically writes `<home>/state/install.json` by temporary file and rename (mode `0600`), with `schema: 1`, `platform`, `role`, `scope`, `label`, `artefactPath`, `nodePath`, `binPath`, and `installedAt`. Dry-run writes no record. An existing record refuses install with exit `4`: if registered, uninstall first; otherwise `stale install record at <path>; run uninstall to clean up`. `install --dry-run` with a stale record previews (exit `0` when the configuration is otherwise valid; configuration errors still exit `3`) and reports the stale record in notes; a *registered* record still refuses with exit `4` in dry-run too. Status defaults to the recorded role, label and scope before config defaults, warning `installed role <r1> differs from config watch.role <r2>` on mismatch. Explicit `status --role` selects that role in the recorded scope. Without a record it uses `--role` or config and reports `registration record: none`. Uninstall defaults to the recorded target; explicit role/scope flags win with a warning when different. It deletes the record after removing that registration (or cleaning a stale record); targeting another registration preserves the original record. If the registration was removed but the artefact file cannot be deleted, uninstall exits `3`, reports `artefact still present at <path>` with a platform-specific hint, runs the platform post-removal command (systemd `daemon-reload`) anyway, and keeps the install record so a later uninstall retries. With otherwise valid non-secret settings but a missing GitHub credential file, `install --dry-run` prints `credential file not found; install will need it` and exits `0`; `uninstall --dry-run` does not require the credential file and also exits `0`.

Windows system wrappers live under `${ProgramData || 'C:\\ProgramData'}\\errmeter\\`; user wrappers remain under the configured home. Both keep `ERRMETER_HOME` pointing to that home. Linux user installs and dry-runs print `note: user services start at login; run 'loginctl enable-linger <user>' to start at boot`. Linux user status runs `loginctl show-user <user> -p Linger` only with `--check` or when a registered service's status is degraded, and reports `yes`, `no`, or `unknown` when unavailable. Otherwise `watcher.linger` is `not probed (use --check)`.
...
- v1.7 note (2026-09-07, #39): strict config resolution validates all non-secret settings (sink repo/URL, notify types, chat_id, plain webhook URLs) before reading any credential file, so a config fault is reported ahead of a missing or unreadable credential and token-less install --dry-run no longer previews a config that watch would reject. Messages, exit codes, fields, formats, config keys and defaults unchanged.
