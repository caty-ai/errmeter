...
Status: **frozen candidate v1.7** for contract-freeze issue #2 (seven review rounds folded in). Input: [design-brief-v0.md](design-brief-v0.md).
...
- **R-F2 At-least-once delivery, idempotent sink.** A spooled event is retried until the sink accepts it — **unboundedly on any host that runs a loop process** (`watch` in either role), and on a host with no loop for `flush_linger_sec` after each emit and again at the next emit (this is the documented limit of a loop-less host; the Epic's outage test runs with a loop); **no unacknowledged event is ever evicted for capacity** (above the hard limit, new events are folded into an overflow log — content is reduced, counts are kept — up to an explicit ceiling `overflow_max_bytes`, beyond which further occurrences are dropped and one alert is raised: this ceiling is the **single documented loss boundary** of the spool; [contract §3.3](contract.md)). The sink MUST tolerate seeing the same event id twice (a crash between "sent" and "marked sent" is expected) and MUST NOT create a second Issue / second comment for the same event id.
