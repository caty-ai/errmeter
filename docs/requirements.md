...
Status: **frozen candidate** for contract-freeze issue #2. Input: [design-brief-v0.md](design-brief-v0.md).
...
- **R-W2** As a watcher, after claiming I run the family's **dispatch hook** (a command the family configures) with the failure as input, and I write the outcome back to the board (attempted / repaired / failed, with the repair PR link if any).
...
- **R-W4** As a watcher, I check heartbeats and raise a **gap alert** when an agent (including other watchers) has been silent longer than its allowed gap. Alerts go to the configured human channels (Telegram / Slack / generic webhook) and are not repeated every interval.
...
- **R-F2 At-least-once delivery, idempotent sink.** A spooled event is retried until the sink accepts it. The sink MUST tolerate seeing the same event id twice (a crash between "sent" and "marked sent" is expected) and MUST NOT create a second Issue / second comment for the same event id.
...
- **R-F6 Dead-man switch.** If **all** watchers are silent, the owner is told by a path that does not depend on a watcher being alive. In v1 that path is: the flush step on any agent machine notices all watcher heartbeats are stale and raises the alert (details in [architecture.md §6](architecture.md)). If every machine in the family is dead at once, nobody can shout; this is an accepted limit of v1 (see X-3).
- **R-F7 Bootstrapping the board.** One command (`errmeter init`) creates the labels the board needs and verifies the token can read and write Issues on the inbox repo and nothing else.
...
- **N-4 Three operating systems.** macOS, Linux, Windows (native, not only WSL). Paths, line endings, file locking, and process spawning MUST behave on all three. The only OS-specific code is boot registration (`install`).
...
- **N-8 Secrets stay home.** Tokens are read from a file with owner-only permissions (POSIX: refuse if group/other readable) or from an environment variable. They are never written to any file errmeter creates and are masked if they appear in any text that errmeter forwards.
- **N-9 Redaction before egress.** All text that leaves the machine passes the redactor ([contract.md §4](contract.md)). Redaction is applied at `emit` time so the spool on disk is already redacted (a stolen spool file does not leak more than the board would).
...
- **X-3** Detecting "the entire family is dead". This needs a third party (an external uptime pinger). v1 exposes enough (watcher heartbeat Issues have a stable title and `updated_at`) for someone to point such a service at it; errmeter does not ship it.
...
| Emit while GitHub is unreachable; nothing lost after recovery | R-F1, R-F2, N-6, N-7 |
| All watchers stopped → heartbeat-gap alert reaches the owner's Telegram | R-W4, R-W5, R-F6, R-O1 |
