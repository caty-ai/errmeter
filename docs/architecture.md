...
If an eligible claim remains pinned for retry because the API budget cannot elect even one claim, the tick reports `watch: api budget too small to elect one claim (max_api_calls_per_pass=<n>, minimum=<m>)`. The minimum is `max_pages_per_list + 7` (17 by default): one listing call, two detail calls, two claim preflight calls, and the claim completion reserve required by contract §5.4. Configuration remains valid; the error identifies a stalled tick.
