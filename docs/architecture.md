...
If an eligible claim remains pinned for retry because the API budget cannot elect even one claim, or candidates exist but `scan_max_confirms` is zero, the tick reports `watch: api budget too small to elect one claim (max_api_calls_per_pass=<n>, minimum=<m>); require max_api_calls_per_pass>=minimum; multi-page listings may need more`. The parseable `minimum=<m>` means the budget must be **at least** `max_pages_per_list + 7` (17 by default): one listing call, two detail calls, two claim preflight calls, and the claim completion reserve required by contract §5.4. Multipage listing or detail reads may require more. This lower bound is not a guarantee that a budget equal to it suffices for every board. The diagnostic is recorded, logged, and emitted once in the process's watch context; a new process, including one with a changed budget, reports again. A private per-tick stall flag keeps the exit status nonzero even when repeat diagnostics are suppressed. A zero confirmation window does not itself set `lookup_incomplete`; incomplete reads and elections retain that existing signal. An empty board with a zero confirmation window remains silent. Configuration remains valid; no startup rejection is added.
